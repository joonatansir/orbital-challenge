...
window.onload = function() {
    ...
    while (openList.length > 0) {
      ...
      graph[current].forEach(function(edge) {
        ...
        if (!costSoFar.hasOwnProperty(next.id) || newCost < costSoFar[next.id]) {
          ...
          openList.push({
            id: next.id,
            cost: newCost + greatCircleDistance(next, route.end)
          });
    ...
    if (route[0] != -1) {
      ...
      parsedData.route = {
        start: {
          ...
          longitude: parseFloat(e[2])
        },
        end: {
          latitude: parseFloat(e[3]),
          longitude: parseFloat(e[4])
        }
      };
    }
    return parsedData;
  ...
  function makeSatellites(satellites, parent) {
    satellites.forEach(function(sat) {
      var s = new THREE.Mesh(new THREE.SphereGeometry(0.02, 10, 10), new THREE.MeshBasicMaterial({
        color: 0xff0000
      }));
  ...
  var start = new THREE.Mesh(new THREE.SphereGeometry(0.05, 10, 10), new THREE.MeshBasicMaterial({
    color: 0x00ff00
  }));
  ...
  var end = new THREE.Mesh(new THREE.SphereGeometry(0.05, 10, 10), new THREE.MeshBasicMaterial({
    color: 0x0000ff
  }));
